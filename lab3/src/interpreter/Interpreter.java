...
public class Interpreter extends firstBaseVisitor<Integer> {
    ...
    @Override
    public Integer visitFunc_def_stat(firstParser.Func_def_statContext ctx) {
        String funcName = ctx.name.getText();
        ArrayList<String> params = new ArrayList<>();
        functions.newSymbol(funcName);
        if(ctx.first != null) {
            for (var param : ctx.ID()) {
                params.add(param.getText());
            }
        }
        functions.setSymbol(funcName, new Pair<>(params, ctx.block()));
        return 0;
    }
